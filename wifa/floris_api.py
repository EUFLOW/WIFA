...
def run_floris(yaml_input):
    ...
    if isinstance(yaml_input, (str, Path)):
        ...
        raise TypeError(f"yaml_input must be a file path or dict, got {type(yaml_input)}")
...
def _read_turbines(fmodel: "FlorisModel", output_dir, out_specs, slice_selection):
    ...
    turbine_nc_filename = turbine_outputs.get("turbine_nc_filename", "turbine_outputs.nc")
                ...
                if data is not None:
                    ...
                    if is_coord:
                        ...
                        if data.ndim == 1:
                            ...
                            data_vars[var_name] = (['wind_direction', 'wind_speed', 'turbine'], data)
                        elif data.ndim == 4:
                            # 4D: (wind_direction, wind_speed, turbulence_intensity, turbine) - WindTIRose
                            data_vars[var_name] = (['wind_direction', 'wind_speed', 'turbulence_intensity', 'turbine'], data)
                        else:
                            print(f"Warning: Unexpected data shape for '{var_name}': {data.shape}; skipping.")
                            continue
                else:
    ...
    if 'time' in coords and coords['time'] is not None:
        ...
        for var_name, (dims, data) in data_vars.items():
            new_dims = tuple('time' if d == 'findex' else d for d in dims)
        ...
        if fmodel.wind_data is not None:
            ...
            if wind_data_type == 'wind_ti_rose':
                final_coords['turbulence_intensity'] = fmodel.wind_data.turbulence_intensities
    else:
    ...
    var_attrs = {
        ...
        'axial_induction': {'units': '-', 'long_name': 'turbine axial induction factor'},
        'turbulence_intensity': {'units': '-', 'long_name': 'turbulence intensity at turbine'},
    }
...
def _read_fields(fmodel: "FlorisModel", output_dir, out_specs, slice_selection):
    ...
    for z_idx, z_height in enumerate(z_list):
        ...
        if z_idx == 0:
            ...
            if "u" in output_vars:
                u_data = np.zeros((len(findex_range), len(z_list), len(y_coords), len(x_coords)))
            if "v" in output_vars:
                v_data = np.zeros((len(findex_range), len(z_list), len(y_coords), len(x_coords)))
            if "w" in output_vars:
                w_data = np.zeros((len(findex_range), len(z_list), len(y_coords), len(x_coords)))
